...
const ui = {
  ...
  questionLabel: (step, total) => `Question ${step} sur ${total}`,
  scaleDescription: (minLabel, maxLabel, min, max) => `(${min} = ${minLabel}, ${max} = ${maxLabel})`,
  nextButton: "Suivant",
...
function downloadJSON(data, meta) {
  ...
  const filename = `fragebogen_${meta.company}_${meta.lastName}_${meta.firstName}_${meta.date}.json`.replace(
    /[^a-zA-Z0-9_\-\.]/g,
    "_"
  );
        ...
        <div className="max-w-4xl mx-auto px-4 py-10 text-center">
          <div className="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left mb-6">
              <input type="text" placeholder={ui.fields.firstName} value={meta.firstName} onChange={(e) => setMeta({ ...meta, firstName: e.target.value })} className="p-2 border border-gray-300 rounded" />
              <input type="text" placeholder={ui.fields.lastName} value={meta.lastName} onChange={(e) => setMeta({ ...meta, lastName: e.target.value })} className="p-2 border border-gray-300 rounded" />
              <input type="text" placeholder={ui.fields.company} value={meta.company} onChange={(e) => setMeta({ ...meta, company: e.target.value })} className="p-2 border border-gray-300 rounded" />
              <input type="text" placeholder={ui.fields.role} value={meta.role} onChange={(e) => setMeta({ ...meta, role: e.target.value })} className="p-2 border border-gray-300 rounded" />
              <input type="email" placeholder={ui.fields.email} value={meta.email} onChange={(e) => setMeta({ ...meta, email: e.target.value })} className="p-2 border border-gray-300 rounded" />
              <input type="tel" placeholder={ui.fields.phone} value={meta.phone} onChange={(e) => setMeta({ ...meta, phone: e.target.value })} className="p-2 border border-gray-300 rounded" />
            </div>
            <button onClick={() => setStep(1)} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
              {ui.startButton}
            </button>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-10">
          <div className="bg-white bg-opacity-90 p-6 rounded-lg shadow-lg text-center">
            ...
            <div className="flex justify-center gap-4 mb-6">
              <button onClick={() => downloadJSON(answers, meta)} className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition">{ui.downloadButton}</button>
              <button onClick={() => setStep(0)} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">{ui.restartButton}</button>
              <button onClick={() => { setStep(0); setAnswers({}); }} className="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition">{ui.resetButton}</button>
            </div>
                ...
                {questions.map((q) => {
                  ...
                  return (
                    <li key={q.id}>
                      ...
                      {q.type === "scale" && answer?.scale && <p className="text-gray-700 mt-1">{ui.labels.answer}: {answer.scale} / {q.scaleMax}</p>}
                      {q.type === "radio" && answer?.selected && <p className="text-gray-700 mt-1">{ui.labels.answer}: {answer.selected}</p>}
                      {q.type === "checkbox" && Array.isArray(answer?.selected) && <p className="text-gray-700 mt-1">{ui.labels.answers}: {answer.selected.join(", ")}</p>}
                      {answer?.comment && <p className="text-sm text-gray-600 mt-1 italic">{ui.labels.comment}: {answer.comment}</p>}
                    </li>
                  );
                })}
              </ul>
